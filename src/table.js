...
import IteratableSelect from './iteratableSelect';
...
const CSS = {
  ...
  content: 'tc-content',
  header: 'tc-header',
  iteratableWrapper: 'tc-iteratable-wrapper',
...
export default class Table {
  ...
  constructor(readOnly, api, data, config) {
    ...
    this.tunes = {
      withHeadings: false,
      iteratable: this.data?.iteratable
    };
  ...
  bindEventsToRow(row) {
    ...
    row.addEventListener("mouseleave", throttled(150, () => {
      if(this.iteratableSelect.element.classList.contains(IteratableSelect.CSS.active)) {
        this.iteratableSelect.element.classList.remove(IteratableSelect.CSS.active)
      }
    }), { passive: true });
  }
  ...
  bindEvents() {
    ...
    if(this.data?.iteratable && this.iteratableSelect) {
      const rows = this.table.querySelectorAll(`.${CSS.row}`)

      for (let i = 0; i < rows.length; i++) {
        this.bindEventsToRow(rows[i])
      }
    }
  ...
  /**
   * Configures and creates the iteratable select
   *
   * @returns {IteratableSelect}
   */
    ...
    return new Toolbox({
      ...
      items: (() => {
        ...
        if(this.data?.iteratable) {
          items.push({
            label: this.api.i18n.t('Iteratable'),
            icon: IconAddBorder,
            hideIf: () => (
              new Promise((res) => {
                setTimeout(() => {
        ...
        items = [...items,
          ...
          },
          {
            ...
            onClick: () => {
              ...
              if(this.iteratableRows.includes(row)) {
                this.iteratableRows = this.iteratableRows.filter(item => item !== row)
              }
  ...
  /**
   * Insert the sell's content by row and column numbers
   *
   * @param {number} row - cell row coordinate
   * @param {number} column - cell column coordinate
   * @param {string} content - cell HTML content
   */
  insertCellContent(row, column, content) {
    const cell = this.getCell(row, column);
  ...
  addRow(index = -1, setFocus = false) {
    ...
    this.bindEventsToRow(insertedRow)
  ...
  createTableWrapper() {
    ...
    this.container = $.make('div', CSS.container);
    this.table = $.make('div', CSS.table);
    this.header = $.make('div', CSS.header);
    this.iteratableWrapper = $.make('div', CSS.header);
    ...
    if(this.data?.iteratable) {
      ...
      this.setIteratableSetting(this.data?.iteratable)
    }
    ...
    if(this.iteratableSelect) {
      this.wrapper.appendChild(this.iteratableSelect.element);
    }
   ...
   * @param {Event} event - mouse move event
   */
  onMouseMoveInRows(event) {
    if(this.iteratableSelect && !this.iteratableSelect?.popover.opened) {
      const {row, column} = this.getHoveredCellInTable(event);

      if(!(this.tunes.withHeadings && row === 1)) {
  ...
  /**
   * Unselect cell, close iteratable select
   *
   * @returns {void}
   */
   ...
   * Makes the first row headings
   *
   * @param {boolean} iteratable
   */
  setIteratableSetting(iteratable) {
    if(this.tunes) {
      this.tunes.iteratable = iteratable;
    }

    const iteratables = this.config?.contents?.iteratables?.[iteratable];
